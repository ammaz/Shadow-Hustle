...
public class ItemManager : MonoBehaviour
{
	...
	public HorizontalScrollSnap horizontalScrollSnapHat;

    public RectTransform contentItemHat;

    public VerticalScrollSnap verticalScrollSnapEquipment;
	public HorizontalScrollSnap horizontalScrollSnapEquipment;
	...
	private void Start()
	{
		...
		beltFollower = GameObject.Find("BeltFollower(Clone)");
	}
	...
	public void GotItem(int type, string id, bool isBelt = false, bool registerNotifs = false)
	{
		...
		if (isBelt)
		{
            ...
            {
                this.youEarnedText.text = "Congratulation！";
                this.gotBeltText.text = "Now, you are a shadow warrior";
            }
            else
            {
                this.youEarnedText.text = "You got";
                this.gotBeltText.text = this.LevelToBeltString(num + 1) + " Belt";
            }
            this.endStory = true;
			return;
		}
		...
		if (type == 1)
		{
			...
			if (this.weaponsById[id].IsOwned())
			{
				...
				this.weaponDuplicateText.text = "Already";
			}
			else
			{
				this.weaponDuplicateText.text = "New";
			...
			this.gotWeaponText.text = this.currentWeapon.GetName();
			if (this.isDuplicate)
			{
		...
		else
		{
			...
			if (this.currentHat.IsOwned())
			{
				...
				StatManager.instance.UpdateStats();
				this.isDuplicate = true;
				this.hatDuplicateText.text = "Already";
			}
			else
			{
				this.hatDuplicateText.text = "New";
				this.currentHat.Own();
			}
			...
			this.gotHatText.text = this.currentHat.GetName();
			if (this.isDuplicate)
			{
	...
	public void GotCoin(int amount)
	{
		...
		this.gotWeaponText.text = amount + " souls";
	...
	public void CloseItemMenu()
	{
		...
		if (beltFollower != null)
		{
			beltFollower.SetActive(true);
		}
	}
    ...
    public void InstanWeaponList()
    {
        ...
        foreach (KeyValuePair<string, Weapon> current in this.weaponsById)
        {
            ...
            string nameItem = current.Value.EnglishName;
